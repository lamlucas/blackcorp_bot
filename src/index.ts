...
/** Số tin chi phí gửi liên tiếp vào cùng nhóm trước khi nghỉ dài. */
const SHEET_PAY_MSG_BATCH_SIZE_DEFAULT = 3;
...
async function broadcastDealerTabs(
  ...
  const msgBatchRaw = parseInt(env.SHEET_PAY_MSG_BATCH_SIZE ?? String(SHEET_PAY_MSG_BATCH_SIZE_DEFAULT), 10);
  const msgBatchSize =
    Number.isFinite(msgBatchRaw) && msgBatchRaw > 0 ? Math.min(msgBatchRaw, 20) : SHEET_PAY_MSG_BATCH_SIZE_DEFAULT;

  /** Nghỉ ngắn giữa mỗi lần gọi Telegram (ms). */
  const telegramGapMs = 400;
  let chiPhiSentInBatch = 0;

  const pauseShort = () => new Promise<void>((resolve) => setTimeout(resolve, telegramGapMs));

  /** Sau mỗi tin chi phí: nghỉ ngắn; cứ msgBatchSize tin thì nghỉ dài (tránh flood Telegram). */
  const afterChiPhiTelegram = async () => {
    await pauseShort();
    chiPhiSentInBatch++;
    if (chiPhiSentInBatch >= msgBatchSize) {
      chiPhiSentInBatch = 0;
      if (pauseBetweenGroupBatches > 0) {
        await new Promise<void>((resolve) => setTimeout(resolve, pauseBetweenGroupBatches));
      }
    }
  };

  /** Trước TỔNG TIỀN + QR — luôn nghỉ dài sau loạt chi phí. */
  const pauseBeforeFooter = async () => {
    chiPhiSentInBatch = 0;
    if (pauseBetweenGroupBatches > 0) {
      await new Promise<void>((resolve) => setTimeout(resolve, pauseBetweenGroupBatches));
    }
  ...
  const pauseBetweenTelegram = pauseShort;
  ...
  const accumulateDebtLocal = async (customerD: string, tongThuNum: number) => {
  ...
  const sendTongTienFooter = async (chatId: string, customerD: string) => {
    ...
    try {
      ...
      await appendSheetPayRunError(
        env,
        runId,
        `Tên khách: ${customerD} — gửi TỔNG TIỀN Telegram lỗi: ${detail.slice(0, 160)}`,
      );
      footerSentThisRun.add(footerRunKey);
      return;
    }
      ...
      for (const entry of sortedEntries) {
        ...
        try {
          ...
          if (tongThuNum > 0 && !forceResend) {
            try {
              await accumulateDebtLocal(customerD, tongThuNum);
            } catch {

...
import { startSheetPayRun } from "./sheet-pay-queue";
...
export default {
  ...
  /** Consumer: gửi công nợ + chi phí theo lô (Queue). */
  async queue(batch: MessageBatch<DebtNotifyJob | SheetPayQueueJob>, env: Env): Promise<void> {
    for (const msg of batch.messages) {
      const body = msg.body;
      if (typeof body === "object" && body !== null && "kind" in body) {
        const { deliverSheetPayQueueJob } = await import("./sheet-pay-queue");
        try {
          await deliverSheetPayQueueJob(env, body as SheetPayQueueJob);
          msg.ack();
        } catch {
          msg.retry();
        }
      ...
      try {
        await deliverDebtNotifyJob(env, body as DebtNotifyJob);
        try {
          await recordDebtSentOk(env, (body as DebtNotifyJob).runId, (body as DebtNotifyJob).maDl);
        } catch {
...
async function handleApi(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  ...
  try {
    ...
    /** Tiếp lô gửi chi phí (Worker tự gọi — tránh timeout waitUntil). */
    if (path === "/api/sheet-pay-continue" && request.method === "POST") {
      const body = (await request.json()) as {
        runId?: string;
        offset?: number;
        token?: string;
      ...
      const { loadSheetPayMeta, processSheetPayChunk } = await import("./sheet-pay-queue");
      const meta = await loadSheetPayMeta(env, runId);
      if (!meta || meta.continueToken !== token) {
        return json({ ok: false, error: "Forbidden" }, 403, request);
      }
      ctx.waitUntil(processSheetPayChunk(env, runId, offset));

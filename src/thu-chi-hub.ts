/**
 * Bot Thu Chi: lệnh nhiều dòng → THU_CHI; Thu → trừ/xóa CONG_NO; note đặc biệt → HH_LOAI_TRU / COC;
 * nhận tin bot Báo cáo « TỔNG TIỀN… » → CONG_NO (telegram-poll).
 */
import { upsertCocFromThuChi, isCocNote } from "./coc-sheet";
...
async function maybeAppendCocAfterThuChi(
  ...
  try {
    const token = await getAccessTokenFromEnv(env);
    const ngay = formatNgayVietnam(unixSec);
    await upsertCocFromThuChi(token, thuChiSpreadsheetId(env), cmd, ngay);
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
...
async function maybeApplyCongNoAfterThu(
  env: Env,
  cmd: ThuChiMultilineCmd,
): Promise<void> {
  ...
  try {
    ...
    const { applyCongNoAfterThuPayment } = await import("./cong-no-sheet");
    await applyCongNoAfterThuPayment(token, spreadsheetId, tabName, cmd.ten, cmd.amount);
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    console.error("cong-no after thu", msg);
  }
}
...
export async function handleThuChiGroupCommand(
  ...
  try {
    ...
    await maybeApplyCongNoAfterThu(env, multilineCmd);

...
export const BAO_CAO_COL = {
  ...
  TIEN_TE: 6,
  QUY_DOI_USD: 7,
  /** Cột I — bot ghi trạng thái gửi: Done / Error */
  NOTE: 8,
  TONG_THU: 9,
  LINK_FILE: 12,
} as const;
...
/** Thiếu trường bắt buộc để gửi tin chi phí đầy đủ. */
export function isBaoCaoRowPaymentIncomplete(row: string[]): boolean {
  const required = [
    BAO_CAO_COL.TAI_KHOAN,
    BAO_CAO_COL.RATE,
    BAO_CAO_COL.TONG_TIEU,
    BAO_CAO_COL.QUY_DOI_USD,
    BAO_CAO_COL.LINK_FILE,
  ];
  return required.some((i) => !String(row[i] ?? "").trim());
}

 ...
 * Chỉ khi mọi dòng lọc (NGÀY+MCC) của khách đều Done mới gửi TỔNG TIỀN + QR: Σ cột I + nợ đầu ngày cột C.
 */
async function broadcastDealerTabs(
  ...
  const sendTongTienFooter = async (chatId: string, customerD: string) => {
    if (!allBaoCaoFilterRowsDoneForCustomer(dataRows, customerD)) return;
    ...
    const sumI = sumTongThuColumnIForCustomerFilterRows(dataRows, customerD);
    let openingC = 0;
    ...
    const htmlThanhToan = formatTongTienCanThanhToanMessage({
      ...
      congNo: formatMoneyForThanhToanLine(openingC),

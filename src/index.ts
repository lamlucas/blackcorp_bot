...
async function handleApi(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  ...
  try {
    ...
    if (path === "/api/coc" && request.method === "PUT") {
      ...
      const body = (await request.json()) as {
        rows?: import("./coc-sheet").CocRowPayload[];
      };
      const { writeCocDataRows, COC_TAB_NAME } = await import("./coc-sheet");
      const rows = Array.isArray(body.rows) ? body.rows : [];
      try {
        const token = await getAccessTokenFromEnv(env);
        const { thuChiSpreadsheetId } = await import("./thu-chi-hub");
        await writeCocDataRows(token, thuChiSpreadsheetId(env), COC_TAB_NAME, rows);
        return json({ ok: true, message: `Đã lưu ${rows.length} dòng tab COC.` }, 200, request);
      } catch (e) {
        const msg = e instanceof Error ? e.message : String(e);
        return json({ ok: false, error: msg }, 502, request);
      }
    ...
    if (path === "/api/analyze-ket-qua-files" && request.method === "POST") {
      if (!(await verifySessionCookie(request, env.SESSION_SECRET))) {
        return json({ ok: false, error: "Chưa đăng nhập" }, 401, request);
      }
      const body = (await request.json()) as {
        files?: { fileName: string; rows: unknown[][] }[];
        config?: {
          campaignCol?: string;
          costCol?: string;
          currencyCol?: string;
      ...
      const config = {
        campaignCol: String(body.config?.campaignCol ?? "").trim(),
        costCol: String(body.config?.costCol ?? "").trim(),
        currencyCol: String(body.config?.currencyCol ?? "").trim(),
        cap1Code: String(body.config?.cap1Code ?? "").trim(),
        cap2Codes: String(body.config?.cap2Codes ?? "").trim(),
        accountNameCol: String(body.config?.accountNameCol ?? "").trim(),
      };
      try {
        const { analyzeKetQuaFiles } = await import("./ket-qua-files");

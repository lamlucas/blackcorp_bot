...
export default {
  ...
  async scheduled(event: ScheduledEvent, env: Env, ctx: ExecutionContext): Promise<void> {
    ctx.waitUntil(runSheetWatcher(env));
  },
};
...
async function runSheetWatcher(env: Env): Promise<void> {
  const { formatSheetRowMessage, hashRow, isRowEmpty } = await import("./format");
  const { sendHtmlMessage, pinMessage, sendPhotoUrl } = await import("./telegram");
  ...
  for (const sheetTitle of titles) {
    ...
    let debtNum = 0;
    if (debtRaw) {
      debtNum = parseFloat(debtRaw.replace(/\s/g, "").replace(",", ".")) || 0;
    }
    const gNum = parseFloat(gStr.replace(/\s/g, "").replace(",", ".")) || 0;
    const thucThuNum = gNum + debtNum;
    const thucThu = debtRaw ? String(thucThuNum) : gStr;

    let debtBlock: Parameters<typeof formatSheetRowMessage>[0]["debtBlock"] | undefined;
    if (debtRaw && debtRaw.trim()) {
      debtBlock = {
        ngay,
        maDl: sheetTitle,
        mcc,
        ...
        rate,
        thucThu,
        congNoCu: debtRaw.trim(),
      };
    }

    const html = formatSheetRowMessage({
      titleLine: titleLine || sheetTitle,
      ngay,
      mcc,
      tongTieu,
      tienTe,

...
export async function fetchAllGroupChatsFromTelegram(
  ...
  let offset = opts?.env?.STORE ? await getTelegramUpdateOffset(opts.env.STORE) : 0;
  let rounds = 0;
  let totalUpdates = 0;
  const maxRounds = 30;
  ...
  for (; rounds < maxRounds; rounds++) {
    ...
    if (opts?.env) {
      const { processTelegramUpdateBatch } = await import("./telegram-poll");
      await processTelegramUpdateBatch(opts.env, batch);
    }

    extractGroupChatsFromUpdates(batch).forEach((v, k) => merged.set(k, v));
  }

  if (opts?.env?.STORE && totalUpdates > 0) {
    await setTelegramUpdateOffset(opts.env.STORE, offset);
  }

...
const ADMIN_TABS = ["dealers", "chat-filter", "sheet-pay", "coc", "debt-notify", "ket-qua", "broadcast"];
...
function renderCocTable(rows) {
  ...
  for (let i = 0; i < cocRowsCache.length; i++) {
    ...
    tr.innerHTML = `
      ...
      <td><input type="text" data-coc="ghiChu" data-idx="${i}" value="${escapeAttr(r.ghiChu)}" /></td>
...
function collectCocRowsFromDom() {
  ...
  return rows.filter((r) => r.ngay || r.ghiChu || r.thu || r.chi);
}
...
async function loadCocTable() {
  ...
  if (!res.ok || !data.ok) {
    if (msg) show(msg, data.error || "Không tải THU_CHI.", true);
    return;
  }
...
document.addEventListener("click", (e) => {
  ...
  const t = e.target;
  if (t instanceof Node && (drawer.contains(t) || toggle?.contains(t))) return;
  closeNavDrawer();
});
for (const btn of document.querySelectorAll(".nav-drawer-item")) {
...
document.getElementById("navRefreshBtn")?.addEventListener("click", () => {
  ...
  else if (saved === "broadcast") void loadBroadcastDealers();
});
...
document.getElementById("sendSheetPayment").addEventListener("click", async () => {
  ...
  const inputs = [...document.querySelectorAll("#sheetPayLinkRows input.sheet-pay-date-inp")].sort(
    (a, b) => Number(a.getAttribute("data-date-index") || 0) - Number(b.getAttribute("data-date-index") || 0)
  );
  const dates = [0, 1, 2].map((i) => String(inputs[i]?.value ?? "").trim());
  const mccInputs = [...document.querySelectorAll("#sheetPayLinkRows input.sheet-pay-mcc-inp")].sort(
    (a, b) => Number(a.getAttribute("data-mcc-index") || 0) - Number(b.getAttribute("data-mcc-index") || 0)
  );
  const mccs = [0, 1, 2].map((i) => String(mccInputs[i]?.value ?? "").trim());
  const filled = dates.filter(Boolean);
  if (filled.length === 0) {
    show(msg, "Điền ít nhất một ô NGÀY.", true);
    return;
  }
  const res = await fetch(url, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    credentials: "include",
    body: JSON.stringify({ dates, mccs }),
  });
  const data = await res.json().catch(() => ({}));
  if (res.status === 401) {

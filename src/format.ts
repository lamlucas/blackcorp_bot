import { escapeHtmlTelegram } from "./telegram";

export function boldValue(v: string): string {
  return `<b>${escapeHtmlTelegram(v)}</b>`;
}

export function line(label: string, value: string): string {
  return `${escapeHtmlTelegram(label)}: ${boldValue(value)}`;
}

export function isRowEmpty(cells: string[]): boolean {
  return cells.every((c) => !String(c ?? "").trim());
}

export function hashRow(cells: string[]): string {
  return cells.map((c) => String(c ?? "").trim()).join("\x1e");
}

/**
 * Chỉ A2–G2: bot ghi đè H2 nên hash không gồm H — tránh gửi lại tin khi H đổi.
 */
export function hashRowDataColumns(cells: string[]): string {
  return hashRow(cells.slice(0, 7));
}

/** Mọi dòng dữ liệu A–G (không H) — dedupe khi đổi bất kỳ dòng nào. */
export function hashPaymentDataRows(dataRows: string[][]): string {
  return dataRows.map((r) => hashRow(r.slice(0, 7))).join("\x1f");
}

/**
 * Parse số từ ô (công nợ, cột G). THỰC THU hiển thị trên Telegram = chuỗi gốc cột G, không dùng hàm này.
 */
export function parseMoneyNumber(raw: string): number {
  let s = String(raw ?? "").trim().replace(/\s/g, "");
  if (!s) return 0;
  s = s.replace(/%/g, "");
  const hasComma = s.includes(",");
  const hasDot = s.includes(".");
  if (hasComma && hasDot) {
    const lastComma = s.lastIndexOf(",");
    const lastDot = s.lastIndexOf(".");
    if (lastComma > lastDot) {
      s = s.replace(/\./g, "").replace(",", ".");
    } else {
      s = s.replace(/,/g, "");
    }
  } else if (hasComma && !hasDot) {
    const p = s.split(",");
    if (p.length === 2 && p[1].length <= 2) {
      s = p[0] + "." + p[1];
    } else {
      s = s.replace(/,/g, "");
    }
  }
  const n = parseFloat(s);
  return Number.isFinite(n) ? n : 0;
}

export function formatNumberForCell(n: number): string {
  if (Number.isInteger(n)) return String(n);
  return String(n);
}

/** Tin thủ công (form web) — MÃ CAMP đầy đủ do Worker ghép tiền tố + tên tab Sheet */
export function formatManualMessage(input: {
  ngay: string;
  mcc: string;
  maCamp: string;
  rate: string;
  ruleLines: string[];
}): string {
  const lines: string[] = [
    line("NGÀY", input.ngay),
    line("MCC", input.mcc),
    line("MÃ CAMP", input.maCamp),
    line("RATE", input.rate),
    "RULE:",
  ];
  for (const r of input.ruleLines) {
    const t = r.trim();
    if (t) lines.push(boldValue(t));
  }
  return lines.join("\n");
}

/**
 * Gộp nhiều dòng (cột A–H) của tab đại lý + link web theo từng dòng.
 * TỔNG TIÊU = ΣC, QUY ĐỔI USD = ΣE, THỰC THU = ΣG, TỔNG THU = công nợ + ΣG.
 */
export function aggregateDealerPaymentForTelegram(
  dataRows: string[][],
  opt: { maDl: string; debtRaw: string; linkFiles: string[] }
): {
  ngay: string;
  maDl: string;
  mcc: string;
  tongTieu: string;
  tienTe: string;
  quyDoiUsd: string;
  rate: string;
  thucThuFromG: string;
  congNoCu: string;
  tongThu: string;
  linkFile: string;
} {
  const debtDisplay = opt.debtRaw.trim();
  const congNoCu = debtDisplay || "0";
  if (dataRows.length === 0) {
    return {
      ngay: "",
      maDl: opt.maDl,
      mcc: "",
      tongTieu: "",
      tienTe: "",
      quyDoiUsd: "",
      rate: "",
      thucThuFromG: "",
      congNoCu,
      tongThu: formatNumberForCell(parseMoneyNumber(opt.debtRaw)),
      linkFile: "",
    };
  }

  const ngayUnique = [...new Set(dataRows.map((r) => String(r[0] ?? "").trim()).filter(Boolean))];
  const ngay = ngayUnique.length ? ngayUnique.join("\n") : "—";

  const mccLines = dataRows.map((r) => String(r[1] ?? "").trim()).filter(Boolean);
  const mcc = mccLines.join("\n");

  let sumC = 0;
  let sumE = 0;
  let sumG = 0;
  for (const r of dataRows) {
    sumC += parseMoneyNumber(String(r[2] ?? ""));
    sumE += parseMoneyNumber(String(r[4] ?? ""));
    sumG += parseMoneyNumber(String(r[6] ?? ""));
  }

  const tienTeLines = dataRows.map((r) => {
    const t = String(r[3] ?? "").trim();
    return t || "—";
  });
  const tienTe = tienTeLines.join("\n");

  /** RATE: mỗi dòng « MCC (cột B) - giá trị cột F », ví dụ MCC1 - 57, MCC2 - 55 */
  const rateTokens: string[] = [];
  for (let i = 0; i < dataRows.length; i++) {
    const mccLabel = String(dataRows[i][1] ?? "").trim() || `Dòng ${i + 2}`;
    const rateVal = String(dataRows[i][5] ?? "").trim();
    rateTokens.push(rateVal ? `${mccLabel} - ${rateVal}` : `${mccLabel} - —`);
  }
  const rate = rateTokens.length ? rateTokens.join(", ") : "—";

  const debtNum = parseMoneyNumber(opt.debtRaw);
  const tongThuNum = sumG + debtNum;

  const linkLines: string[] = [];
  for (let i = 0; i < dataRows.length; i++) {
    const label = String(dataRows[i][1] ?? "").trim() || `Dòng ${i + 2}`;
    const u = String(opt.linkFiles[i] ?? "").trim();
    linkLines.push(u ? `${label} - ${u}` : `${label} - —`);
  }

  return {
    ngay,
    maDl: opt.maDl,
    mcc: mcc || "—",
    tongTieu: formatNumberForCell(sumC),
    tienTe,
    quyDoiUsd: formatNumberForCell(sumE),
    rate,
    thucThuFromG: formatNumberForCell(sumG),
    congNoCu,
    tongThu: formatNumberForCell(tongThuNum),
    linkFile: linkLines.join("\n"),
  };
}

/** Một khối — đúng cú pháp: THỰC THU = nội dung cột G; TỔNG THU = G + công nợ (và ghi cột H). Không thêm dòng tiêu đề A1 (tránh trùng chữ NGÀY). */
export function formatSheetRowMessage(opt: {
  ngay: string;
  maDl: string;
  mcc: string;
  tongTieu: string;
  tienTe: string;
  quyDoiUsd: string;
  rate: string;
  /** Giá trị hiển thị THỰC THU = đúng như cột G trên Sheet */
  thucThuFromG: string;
  congNoCu: string;
  /** TỔNG THU = số (G + nợ) */
  tongThu: string;
  /** URL từ form web « Send Thanh Toán » */
  linkFile: string;
}): string {
  const parts: string[] = [];
  parts.push(line("NGÀY", opt.ngay));
  parts.push(line("MÃ ĐL", opt.maDl));
  parts.push(line("MCC", opt.mcc));
  parts.push(line("TỔNG TIÊU", opt.tongTieu));
  parts.push(line("TIỀN TỆ", opt.tienTe));
  parts.push(line("QUY ĐỔI USD", opt.quyDoiUsd));
  parts.push(line("RATE", opt.rate));
  parts.push(line("THỰC THU", opt.thucThuFromG));
  parts.push(line("CÔNG NỢ CŨ", opt.congNoCu));
  parts.push(line("TỔNG THU", opt.tongThu));
  parts.push(line("LINK FILE", opt.linkFile.trim() || "—"));
  return parts.join("\n");
}

/**
 * Đại lý không tick nhưng có nợ trên tab CONG_NO (cột A = MÃ ĐL, cột B = NỢ CŨ).
 * Chuỗi A/B đọc từ Sheet; khi khớp tên tab đại lý, hiển thị đúng nhãn MÃ ĐL / NỢ CŨ.
 */
export function formatDebtOnlyNotify(opt: { maDl: string; noCu: string }): string {
  return [line("MÃ ĐL", opt.maDl), line("NỢ CŨ", opt.noCu)].join("\n");
}

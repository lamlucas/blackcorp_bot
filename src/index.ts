...
async function handleApi(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  ...
  try {
    ...
    if (path === "/api/send-sheet-payment" && request.method === "POST") {
      ...
      return json(
        { ok: true, message: `Đang gửi thanh toán tới ${selectedTabs.length} nhóm đã chọn…` },
        202,
        request
      );
    }
...
/**
 * Gửi tin theo hàng 2 từng tab đại lý + công nợ sheet riêng.
 * Nếu cột B có giá trị cho đúng tên tab trong sheet công nợ → vẫn gửi kể cả khi hàng 2 đại lý trống (không có chi tiêu).
 * skipKvDedup=true (Gửi chi phí): không dedupe KV trước khi gửi.
 */
async function broadcastDealerTabs(
  ...
  for (const sheetTitle of titles) {
    ...
    if (selected.size > 0 && !selected.has(sheetTitle)) continue;

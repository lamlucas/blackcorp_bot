...
/** all = mọi lệnh; thuchi = Thu/Chi + chấm công + /mail; none = bỏ qua. */
...
export async function handleGroupTelegramText(
  ...
  if (scope === "all" || scope === "thuchi") {
    const { isMailTelegramCommand, handleMailTelegramCommand } = await import("./mail-telegram");
    if (isMailTelegramCommand(text)) {
      if (!fromIsBot) await handleMailTelegramCommand(env, chatId);
      return;
    }

    const { handleChamCongGroupMessage } = await import("./cham-cong-hub");
    const handledChamCong = await handleChamCongGroupMessage(env, chatId, text, {
      from: msg?.from,
      unixSec: msg?.date,
    });
    if (handledChamCong) return;

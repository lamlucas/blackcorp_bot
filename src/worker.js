...
function rowHasSyncableData(row) {
  for (let i = 0; i <= 6; i += 1) {
...
async function hashRowSignature(row) {
  const parts = [];
  for (let i = 0; i <= 6; i += 1) {
...
async function syncSheetToTelegram(env, options = {}) {
  ...
  for (const group of groups) {
    ...
    const currentHashes = await Promise.all(dataRows.map((r) => hashRowSignature(r)));
    ...
    if (prevHashes === null) {
      ...
      if (catchUp && dataRows.length > 0) {
        for (const row of dataRows) {
          if (!rowHasSyncableData(row)) continue;
    ...
    summary.push({
      ...
      note:
        sentCount === 0
          ? "Khong co dong moi hoac khong co o nao trong A-G thay doi"

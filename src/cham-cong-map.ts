...
/** Tên hiển thị Telegram → tên tab Sheet (vd Subeo → SUBEO). */
 ...
 * So khớp không phân biệt hoa thường: tên Telegram hoặc tên tab (vd Subeo ↔ SUBEO).
 */
...
/** Tên tab hợp lệ từ tên nhân viên (BLACK, SUBEO). */
...
/** Cập nhật map KV nếu tên tab trên Sheet khác (vd SU_BEO → SUBEO). */
export async function reconcileChamCongEmployeeMapTabs(
  accessToken: string,
  spreadsheetId: string,
  map: Record<string, string>,
  kv?: KVNamespace,
): Promise<Record<string, string>> {
  const { resolveChamCongTabTitle } = await import("./cham-cong-sheet");
  const out: Record<string, string> = {};
  let changed = false;
  for (const [telegramName, tabName] of Object.entries(map)) {
    try {
      const resolved = await resolveChamCongTabTitle(accessToken, spreadsheetId, tabName);
      out[telegramName] = resolved;
      if (resolved !== tabName) changed = true;
    } catch {
      out[telegramName] = tabName;
    }
  }
  if (changed && kv) await setChamCongEmployeeMap(kv, out);
  return out;
}

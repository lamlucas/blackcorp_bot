...
async function handleApi(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  ...
  try {
    ...
    if (path === "/api/send-sheet-payment" && request.method === "POST") {
      ...
      const hasAny = TINH_TIEN_TAB_NAMES.some((t) => allTitles.includes(t));
      if (!hasAny) {
        return json(
          {
            ok: false,
            error:
              "Sheet chính không có tab TINH_TIEN1…TINH_TIEN6. Tạo ít nhất một tab đúng tên trên file chi phí.",
          },
          400,
          request
        );
      ...
      return json(
        {
          ok: true,
          message: "Đang gửi chi phí (TINH_TIEN1…6): mỗi dòng dữ liệu một tin nhóm + QR…",
        },
        202,
        request
      );
 ...
 * Mỗi dòng dữ liệu (A–I) → một tin Telegram + một ảnh QR; ghi TỔNG THU vào cột J cùng dòng.
 * LINK FILE k = link gửi kèm mọi dòng của tab TINH_TIENk (form có 6 ô).
 * skipKvDedup=true: luôn gửi (panel web); false: chỉ gửi khi hash dòng đổi (cron nếu sau này dùng).
 */

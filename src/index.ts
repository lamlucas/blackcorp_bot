 ...
 * Nếu cùng tên cột D xuất hiện từ ≥2 dòng (các tab có LINK FILE), sau tin đầu tiên gửi thành công cho tên đó thêm một tin « TỔNG THU CỘNG LẠI » = tổng cột I mọi dòng trùng D (không lọc CONG_NO).
 */
async function broadcastDealerTabs(
  ...
  const duplicateDNorms = duplicateCustomerDNormSet(allTabDataRows);
  const sentTongThuCongLai = new Set<string>();
  ...
  for (let ti = 0; ti < TINH_TIEN_TAB_NAMES.length; ti++) {
    ...
    for (let i = 0; i < dataRows.length; i++) {
      ...
      try {
        ...
        if (
          dNorm &&
          duplicateDNorms.has(dNorm) &&
          !sentTongThuCongLai.has(dNorm)
        ) {
          const tongCongI = sumThucThuColumnIForCustomerD(
            allTabDataRows,
            customerD
          );
          const htmlCong = formatTongThuCongLaiMessage({
            maDlTenKhach: customerD,
            tongCongLaiI: formatNumberForCell(tongCongI),
          });
          try {
            await sendHtmlMessage(env.TELEGRAM_BOT_TOKEN, chatId, htmlCong);
            sentTongThuCongLai.add(dNorm);
          } catch {
            /* tin phụ lỗi không hoàn tác tin chính */
          }
          await pauseBetweenTelegram();
        }
      } catch {

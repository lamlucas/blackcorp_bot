      ...
      return json(
        {
          ok: true,
          message:
            "Đang gửi chi phí: chỉ các tab TINH_TIEN có LINK FILE + dòng có cột D khớp nhóm tab (map Đại lý)…",
        },
        202,
        request
      );
...
/**
 * Gửi chi phí: các tab TINH_TIEN1…6 trên MAIN_SPREADSHEET_ID.
 * Chỉ đọc / tổng hợp tab nào có ô LINK FILE tương ứng (1…6) không trống.
 * Mỗi dòng (A–I) → một tin + QR; ghi J; cột D phải khớp nhóm tab (map Đại lý: tên = D → cùng Chat ID tab).
 * LINK FILE k = link gửi kèm mọi dòng của tab TINH_TIENk.
 * skipKvDedup=true: luôn gửi (panel web); false: chỉ gửi khi hash dòng đổi.
 */
async function broadcastDealerTabs(
  ...
  type TabSnap = { chatId: string | null; dataRows: string[][] };
  ...
  for (let ti = 0; ti < TINH_TIEN_TAB_NAMES.length; ti++) {
    ...
    if (!linkByTabIndex[ti]) {
      tabSnap[tabName] = { chatId: null, dataRows: [] };
      allTabDataRows[tabName] = [];
      continue;
    }
    try {
      const r = await readTabRows(token, env.MAIN_SPREADSHEET_ID, tabName);
      const chatId = resolveChatId(tabName, r.chatId, dealerMap);
      tabSnap[tabName] = { chatId, dataRows: r.dataRows };
      allTabDataRows[tabName] = r.dataRows;
    } catch {
      tabSnap[tabName] = { chatId: null, dataRows: [] };
      allTabDataRows[tabName] = [];
    }
  ...
  for (let ti = 0; ti < TINH_TIEN_TAB_NAMES.length; ti++) {
    ...
    for (let i = 0; i < dataRows.length; i++) {
      ...
      if (!customerColumnDMatchesTabChat(customerD, chatId, dealerMap)) continue;

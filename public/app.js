...
function setTab(tab) {
  const tabs = ["dealers", "chat-filter", "sheet-pay", "debt-notify", "broadcast"];
...
async function loadText() {
  try {
    ...
    if (t.debtNotifyButton) document.getElementById("sendDebtNotify").textContent = t.debtNotifyButton;
...
document.getElementById("sendDebtNotify").addEventListener("click", async () => {
  const msg = document.getElementById("debtNotifyMsg");
  msg.hidden = true;
  const url = apiUrl("/api/send-debt-notify");
  if (!url) {
    show(msg, "Chỉ gửi được khi site chạy trên máy chủ (không dùng file://).", true);
    return;
  }
  ...
  show(msg, data.message || "Đã nhận — bot đang gửi công nợ…", false);
});

...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === "/api/telegram-webhook") {
      ...
      if (request.method === "POST") {
        const { handleTelegramWebhookPost } = await import("./telegram-webhook-route");
        return handleTelegramWebhookPost(request, env);
      }
    }
  ...
  async scheduled(event: ScheduledEvent, env: Env, ctx: ExecutionContext): Promise<void> {
    /** Mỗi 2 phút: getUpdates (không webhook) — Thu: + TỔNG TIỀN → CONG_NO */
    if (event.cron === "*/2 * * * *") {
...
async function handleApi(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  ...
  try {
    ...
    if (path === "/api/telegram-group-chats" && request.method === "GET") {
      ...
      try {
        const { chats, rounds, updatesConsumed, warning } =
          await fetchAllGroupChatsFromTelegram(tok, { env });
    ...
    /** Gửi chi phí: tab TINH_TIEN1…9 trên MAIN_SPREADSHEET_ID, mỗi dòng một tin (+ QR + ghi J). */
        ...
        return json(
          {
            ok: false,
            error:
              "Sheet chi phí không có tab TINH_TIEN1…TINH_TIEN9. Tạo tab đúng tên trên file.",
          },
          400,
          request
        );
 ...
 * Sau khi gửi hết tin chi phí từng dòng (kể cả cùng tên cột D nhiều tab): một tin « TỔNG TIỀN CẦN THANH TOÁN » = Σ cột I + công nợ (A=D); rồi ảnh QR (một lần mỗi tên D / nhóm).
 */
async function broadcastDealerTabs(
  ...
  /** Sau khi gửi xong mọi dòng chi phí của nhóm (chatId + tên D): gửi tổng thanh toán rồi QR. */

...
async function handleApi(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  ...
  try {
    ...
    /** Gửi chi phí: chỉ tab được tick → báo cáo đầy đủ (+ QR + H). Tin chỉ công nợ → panel /api/send-debt-notify. */
    if (path === "/api/send-sheet-payment" && request.method === "POST") {
      ...
      const body = (await request.json()) as { linkFile?: string; selectedTabs?: string[] };
    ...
    /**
     * Đọc toàn bộ tab CONG_NO (A2:B): mỗi dòng có A + B → gửi vào nhóm của tab đại lý trên MAIN có tên trùng cột A (+ QR + ghim).
     * Không cần tick chọn; không KV / không ghi H.
     */
    if (path === "/api/send-debt-notify" && request.method === "POST") {
      if (!(await verifySessionCookie(request, env.SESSION_SECRET))) {
        return json({ ok: false, error: "Chưa đăng nhập" }, 401, request);
      }

      ctx.waitUntil(broadcastDebtFromCongNoTab(env));
      return json(
        {
          ok: true,
          message:
            "Đang gửi thông báo công nợ cho từng dòng CONG_NO có khớp tab đại lý trên Sheet chính…",
        },
        202,
        request
      );
...
async function broadcastDealerTabs(
  ...
  for (const sheetTitle of titlesOrdered) {
    ...
    const congNoLabel = debtDisplay || "0";
...
/** Mỗi dòng CONG_NO (A,B): gửi tin nợ vào nhóm của tab MAIN có tên = cột A (sau trim). */
async function broadcastDebtFromCongNoTab(env: Env): Promise<void> {

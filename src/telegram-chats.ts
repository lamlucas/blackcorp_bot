 ...
 */
export async function fetchAllGroupChatsFromTelegram(botToken: string): Promise<{
  chats: GroupChatRow[];
  rounds: number;
  ...
  for (; rounds < maxRounds; rounds++) {
    const url = `https://api.telegram.org/bot${botToken}/getUpdates?limit=100&timeout=0&offset=${offset}`;
    ...
    if (!data.ok) {
      ...
      if (code === 401 || /unauthorized/i.test(String(msg))) {
        throw new Error(
          "Telegram từ chối token (Unauthorized). Vào Cloudflare → Worker → Secrets → kiểm tra TELEGRAM_BOT_TOKEN đúng với @BotFather (copy lại, không thừa dấu cách). Nếu vừa đổi token — deploy lại hoặc Save secret."
        );
      }
      if (/webhook/i.test(String(msg)) || code === 409) {
        msg +=
          " Bot đang dùng Webhook: trong trình duyệt mở https://api.telegram.org/bot<TOKEN>/deleteWebhook?drop_pending_updates=true — hoặc tắt webhook hosting khác — rồi thử lại.";
      }
      throw new Error(msg);
    }
    ...
    for (const upd of batch) {
      ...
      if (typeof uid === "number") offset = uid + 1;
    }

 ...
 * Mỗi dòng → tin chi phí (CÔNG NỢ = B + cột I tích lũy) → ghi J = Done, cộng I vào CONG_NO cột B.
 * Chỉ khi mọi MCC (TỔNG THU > 0) đã gửi tin chi phí thành công vào nhóm mới gửi TỔNG TIỀN + QR.
 */
async function broadcastDealerTabs(
  ...
  const {
    ...
    listPayableFilterRowsForCustomer,
  ...
  /** Khách vừa gửi đủ mọi dòng payable trong lượt này → luôn gửi TỔNG TIỀN + QR. */
  const footerReadyThisRun = new Set<string>();
  const openingCCache = new Map<string, number>();

  const getOpeningCForCustomer = async (customerD: string): Promise<number> => {
    const key = customerD.trim().toLowerCase();
    if (openingCCache.has(key)) return openingCCache.get(key)!;
    let openingC = 0;
    try {
      const { ensureCongNoOpeningDebtColumnC } = await import("./cong-no-sheet");
      const opening = await ensureCongNoOpeningDebtColumnC(
        token,
        debtSpreadsheetId,
      ...
      if (opening) openingC = opening.openingC;
    } catch {
      /* mặc định 0 */
    }
    openingCCache.set(key, openingC);
    return openingC;
  };

  /** Mọi dòng TỔNG THU > 0 đã gửi chi phí (cột J = Done). */
  const allPayableRowsSentToGroupForCustomer = (customerD: string): boolean => {
    const payable = listPayableFilterRowsForCustomer(filterRows, customerD);
  ...
  const sendTongTienFooter = async (chatId: string, customerD: string) => {
    ...
    /* Kiểm tra Done trên bộ nhớ — tránh reload Sheet ghi đè trước khi API kịp cập nhật cột J. */
    if (!allPayableRowsSentToGroupForCustomer(customerD)) return;

    await mergeFilterRowsFromSheet();
    if (!allPayableRowsSentToGroupForCustomer(customerD)) return;

    const kvFooter = kvPayTongKey(debtSpreadsheetId, customerD, filterSig);
    const custKey = customerD.trim().toLowerCase();
    const unlockFooter = forceResend || footerReadyThisRun.has(custKey);

    let openingC = 0;
    let maDlKey: string | null = null;
    try {
      const { ensureCongNoOpeningDebtColumnC } = await import("./cong-no-sheet");
      const opening = await ensureCongNoOpeningDebtColumnC(
        token,
      ...
      if (opening) {
        openingC = opening.openingC;
        maDlKey = opening.maDlKey;
      }
    } catch {
      /* vẫn gửi tin nếu đọc nợ đầu ngày lỗi */
    }

    const breakdown = buildTongTienBreakdownForCustomer(filterRows, customerD, openingC);
    const total = breakdown.total;
    if (total <= 0) {
      if (!opts.skipKvDedup) await store.put(kvFooter, "1");
      return;
    }

    if (maDlKey) {
      try {
        const { setCongNoDebtColumnBTotal } = await import("./cong-no-sheet");
        const totalDisplay = formatMoneyForThanhToanLine(total);
        await setCongNoDebtColumnBTotal(
          token,
          debtSpreadsheetId,
          env.DEBT_TAB_NAME.trim(),
          maDlKey,
        ...
        debtMap.set(maDlKey, totalDisplay);
      } catch {
        /* thử gửi tin dù ghi sheet lỗi */
      }
    }

    if (!unlockFooter && !opts.skipKvDedup) {
      const prevFooter = await store.get(kvFooter);
      if (prevFooter === "1") {
    ...
    const htmlThanhToan = formatTongTienCanThanhToanMessage({
      openingC: formatMoneyForThanhToanLine(openingC),
  ...
  for (let batchStart = 0; batchStart < paymentGroups.length; batchStart += SHEET_PAY_GROUP_BATCH_SIZE) {
    ...
    for (const group of batch) {
      ...
      const openingCForGroup = forceResend ? await getOpeningCForCustomer(customerD) : 0;
      ...
      for (const entry of sortedEntries) {
        ...
        if (forceResend && tongThuNum > 0) {
          ...
          congNoSauMcc = formatDebtDisplayForTelegram(
            formatMoneyForThanhToanLine(Math.round((openingCForGroup + sumUpTo) * 100) / 100),
          );
        ...
        try {
          ...
          if (allPayableRowsSentToGroupForCustomer(customerD)) {
            footerReadyThisRun.add(customerD.trim().toLowerCase());
          }
  ...
  for (const target of customerTargets.values()) {
    if (allPayableRowsSentToGroupForCustomer(target.customerD)) {
      footerReadyThisRun.add(target.customerD.trim().toLowerCase());
    }

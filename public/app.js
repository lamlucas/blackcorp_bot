...
const ADMIN_TABS = ["dealers", "chat-filter", "sheet-pay", "coc", "cham-cong", "debt-notify", "ket-qua", "ket-qua-files", "broadcast"];
let chamCongSelectedTab = "";
const CHAM_CONG_TEMPLATE_TAB = "SU_BEO";
...
function renderChamCongTable(rows) {
  ...
  for (const row of rows) {
    ...
    tr.append(tdNgay, tdTick, tdTien);
...
function renderChamCongEmployeeTable(employees) {
  ...
  for (const e of employees) {
    ...
    const isTemplate = e.tabName === CHAM_CONG_TEMPLATE_TAB;
    if (!isTemplate) {
...
function updateChamCongDeleteButton(tabName) {
  ...
  if (!btn) return;
  const blocked = !tabName || tabName === CHAM_CONG_TEMPLATE_TAB;
  btn.disabled = blocked;
  btn.title = blocked ? "Không xóa được tab mẫu SU_BEO" : `Xóa tab ${tabName}`;
}
...
async function loadChamCongPanel() {
  ...
  const tienInput = document.getElementById("chamCongTienUngMacDinh");
  if (tienInput) tienInput.value = data.tienUngMacDinh ?? "";
...
async function deleteChamCongEmployee(tabName) {
  ...
  if (name === CHAM_CONG_TEMPLATE_TAB) {
    if (msg) show(msg, "Không thể xóa tab mẫu SU_BEO.", true);
    return;
  }
...
async function saveChamCongTienUng() {
  const msg = document.getElementById("chamCongMsg");
  ...
  const tienUngMacDinh = document.getElementById("chamCongTienUngMacDinh")?.value ?? "";
  if (!url || !tabName) {
  ...
  const res = await fetch(url, {
    ...
    headers: { "Content-Type": "application/json" },
    credentials: "include",
    body: JSON.stringify({ tabName, tienUngMacDinh }),
  });
  const data = await res.json().catch(() => ({}));
  if (!res.ok || !data.ok) {
...
document.getElementById("chamCongSave")?.addEventListener("click", () => void saveChamCongTienUng());

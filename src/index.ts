...
async function handleApi(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  ...
  try {
    ...
    /** Gửi chi phí: tab được tick → báo cáo đầy đủ; tab không tick nhưng có nợ → tin công nợ (không QR/H). */
    if (path === "/api/send-sheet-payment" && request.method === "POST") {
      ...
      ctx.waitUntil(broadcastDealerTabs(env, { skipKvDedup: true, linkFile, selectedTabs }));
      return json(
        {
          ok: true,
          message: `Đang gửi: báo cáo đầy đủ cho ${selectedTabs.length} đại lý đã chọn; đồng thời báo công nợ cho các đại lý khác có nợ (không chọn)…`,
        },
        202,
        request
      );
 ...
 * - Tab không tick nhưng có nợ (CONG_NO cột A/B): tin MÃ ĐL + NỢ CŨ + 2 ảnh QR + ghim tin text (không ghi H).
 * skipKvDedup=true: không dedupe KV trước khi gửi (chỉ áp dụng nhánh báo cáo đầy đủ).
 */
async function broadcastDealerTabs(
  env: Env,
  opts: { skipKvDedup: boolean; linkFile: string; selectedTabs?: string[] }
): Promise<void> {
  if (!env.STORE) return;
  ...
  for (const sheetTitle of titles) {
    ...
    const isSelected = selected.has(sheetTitle);

    if (!isSelected) {
      if (!debtDisplay) continue;

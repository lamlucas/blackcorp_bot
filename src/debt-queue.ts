...
export interface DebtNotifyJob {
  chatId: string;
  maDl: string;
  noCuDisplay: string;
  runId: string;
...
/** Thu thập các nhóm cần gửi từ CONG_NO + Sheet chính (một lần đọc). */
export async function collectDebtNotifyJobs(env: Env): Promise<DebtNotifyJob[]> {
  let token: string;
  try {

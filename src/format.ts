...
export function sumEligibleThucThuIForCustomer(
  ...
  for (const rows of Object.values(tabRows)) {
    for (const r of rows) {
      ...
      if (rd === dNorm) sum += parseMoneyNumber(String(r[9] ?? r[8] ?? ""));
    }
  }
  return sum;
}
...
const RE_TONG_TIEN =
  /TỔNG\s*TIỀN\s*CẦN\s*THANH\s*TOÁN\s*:\s*([\d.,\s]+)\s*\+\s*([\d.,\s]+)\s*=\s*([\d.,\s]+)/iu;

/** Parse dòng « TỔNG TIỀN CẦN THANH TOÁN: a + b = c » — trả về tổng (phần sau dấu =). */
export function parseTongTienCanThanhToanTotal(text: string): string | null {
  ...
  if (!m) return null;
  const totalRaw = String(m[3] ?? "").trim();
...
/**
 * Sau Thu:/Chi: — chênh lệch số tiền lệnh và nợ cột B ≤ tolerance → xóa B;
 * chênh > tolerance → B mới = B − số tiền lệnh (ghi dấu phẩy).
 */
export function computeCongNoAfterThuChi(
  ...
  if (diff <= tolerance) return { action: "clear" };
  const remaining = Math.round((b - paymentAmount) * 100) / 100;
  if (remaining <= tolerance) return { action: "clear" };
...
/** Một dòng tab BAO_CAO_TK — TỔNG THU = cột I; LINK FILE = cột M; không có THỰC THU. */

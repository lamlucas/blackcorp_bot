...
async function handleApi(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  ...
  try {
    ...
    if (path === "/api/send-sheet-payment" && request.method === "POST") {
      ...
      const { parseFilterSlotsFromPanel, BAO_CAO_TK_TAB_NAME } = await import("./bao-cao-tk");
      ...
      return json(
        {
          ok: true,
          message: `Đang gửi chi phí: ${filterSlots.length} bộ lọc (${filterDates.join(", ")}${mccHint}) — tab BAO_CAO_TK…`,
          filterDates,
          filterSlots,
        },
        202,
    ...
    /** Đọc tab THU_CHI (panel Tiền cọc). */
    if (path === "/api/coc" && request.method === "GET") {
      ...
      try {
        ...
        const { readThuChiSheetRows, THU_CHI_TAB_NAME } = await import("./coc-sheet");
        const { thuChiSpreadsheetId } = await import("./thu-chi-hub");
        const rows = await readThuChiSheetRows(token, thuChiSpreadsheetId(env), THU_CHI_TAB_NAME);
        return json(
          {
            ok: true,
            spreadsheetId: thuChiSpreadsheetId(env),
            tabName: THU_CHI_TAB_NAME,
            rows: rows.map((r) => ({
    ...
    /** Ghi lại tab THU_CHI từ panel (A2:D). */
    if (path === "/api/coc" && request.method === "PUT") {
      if (!(await verifySessionCookie(request, env.SESSION_SECRET))) {
        return json({ ok: false, error: "Chưa đăng nhập" }, 401, request);
      }
      const body = (await request.json()) as {
        rows?: import("./coc-sheet").ThuChiRowPayload[];
      };
      const { writeThuChiDataRows, THU_CHI_TAB_NAME } = await import("./coc-sheet");
      const rows = Array.isArray(body.rows) ? body.rows : [];
      try {
        const token = await getAccessTokenFromEnv(env);
        const { thuChiSpreadsheetId } = await import("./thu-chi-hub");
        await writeThuChiDataRows(token, thuChiSpreadsheetId(env), THU_CHI_TAB_NAME, rows);
        return json({ ok: true, message: `Đã lưu ${rows.length} dòng tab THU_CHI.` }, 200, request);
      } catch (e) {
        const msg = e instanceof Error ? e.message : String(e);
        return json({ ok: false, error: msg }, 502, request);
      }

...
/** Chuẩn bị lô gửi → lưu KV → chạy lô đầu (mỗi lô tối đa chunkSize dòng, tự gọi tiếp). */
...
export async function processSheetPayChunk(
  ...
  for (const job of slice) {
    try {
      await deliverSheetPayRowJob(env, meta, job);
      meta.rowsSent++;
      sentInChunk++;
    } catch (e) {
      const detail = e instanceof Error ? e.message : String(e);
  ...
  if (nextOffset < meta.rowJobs.length) {
    if (sentInChunk >= meta.chunkSize && meta.batchPauseMs > 0) {
...
async function deliverSheetPayRowJob(
  ...
  let debtMap = await getDebtMap(token, debtSpreadsheetId, env.DEBT_TAB_NAME);

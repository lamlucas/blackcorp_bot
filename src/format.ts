...
/**
 * Đếm số dòng (mọi tab TINH_TIEN đã nạp) theo cột D (trim, so khớp không phân biệt hoa thường).
 */
export function countRowsByCustomerDNorm(
  tabRows: Record<string, string[][]>
): Map<string, number> {
  const m = new Map<string, number>();
  for (const rows of Object.values(tabRows)) {
    for (const r of rows) {
      const d = String(r[3] ?? "").trim().toLowerCase();
      if (!d) continue;
      m.set(d, (m.get(d) ?? 0) + 1);
    }
  }
...
/** Tên cột D xuất hiện từ 2 dòng trở lên (có thể khác tab hoặc cùng tab). */
export function duplicateCustomerDNormSet(
  tabRows: Record<string, string[][]>
): Set<string> {
  const counts = countRowsByCustomerDNorm(tabRows);
...
/**
 * Tổng cột I (THỰC THU) mọi dòng có cột D trùng tên — không lọc CONG_NO (dùng cho tin « TỔNG THU CỘNG LẠI »).
 */
...
/** Tin bổ sung khi cột D trùng nhiều dòng: tổng các ô I gộp lại. */
export function formatTongThuCongLaiMessage(opt: {
  maDlTenKhach: string;
  tongCongLaiI: string;
}): string {
  return [
    line("TỔNG THU CỘNG LẠI", opt.tongCongLaiI || "0"),
    `${escapeHtmlTelegram("Tổng cột I (THỰC THU) mọi dòng có cùng tên cột D")}: ${boldValue(
      opt.maDlTenKhach || "—"
    )}`,
  ].join("\n");
}

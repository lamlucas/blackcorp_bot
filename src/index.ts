    ...
    if (path === "/api/telegram-group-chats" && request.method === "GET") {
      ...
      try {
        ...
        try {
          ...
          if (/webhook/i.test(msg) && byId.size === 0) {
            return json(
              {
                ok: false,
                error:
                  "Bot Thu Chi đang bật Webhook — gửi một tin trong từng nhóm đại lý (vd /start), đợi vài giây rồi bấm lại; hoặc nhập Chat ID tay.",
              },
              400,
              request
            );
        ...
        return json(
          {
            ...
            warning:
              scanWarning ||
              (chats.length === 0
                ? "Chưa có nhóm trong bộ nhớ. Gửi tin trong nhóm (bot Thu Chi phải có trong nhóm) rồi bấm lại."
                : undefined),
            note:
              "Ưu tiên nhóm bot Thu Chi đã thấy qua webhook. Không chạy lại lệnh Thu/Chi khi bấm nút này.",
          },
          200,
          request
        );

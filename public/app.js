...
async function loadText() {
  try {
    ...
    if (t.ketQuaFilesLblLink)
      document.getElementById("ketQuaFilesLblLink").textContent = t.ketQuaFilesLblLink;
...
document.getElementById("ketQuaFilesForm").addEventListener("submit", async (e) => {
  ...
  const url = apiUrl("/api/run-account-back-filter");
  if (!url) {
    show(msg, "Chỉ chạy được khi site trên Worker.", true);
    return;
  }
  ...
  show(msg, "Đang đọc Sheet và ghi báo cáo…", false);
  msg.hidden = false;

  const body = {
    spreadsheetUrlOrId: document.getElementById("ketQuaFilesSpreadsheetUrl").value.trim(),
  ...
  if (!res.ok || !data.ok) {
    show(msg, data.error || "Chạy lọc thất bại", true);
    return;
  }
  show(msg, data.message || "Đã xong.", false);
});

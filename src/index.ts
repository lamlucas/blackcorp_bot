...
async function broadcastDealerTabs(
  ...
  const writeRowNote = async (sheetRow: number, status: string) => {
    ...
    try {
      ...
      /* ghi Note lỗi không chặn gửi tin khác */
    }
  };
  ...
  for (const entry of filterRows) {
    ...
    if (!opts.skipKvDedup) {
      const prev = await store.get(kvK);
      if (prev === "1") continue;
    }
  ...
  /** Done = tin chi phí MCC đã gửi Telegram thành công (KV payrow). */
  const allPayableRowsSentToGroupForCustomer = async (customerD: string): Promise<boolean> => {
    ...
    for (const entry of payable) {
      const rowHash = hashBaoCaoTkRowSnapshot(entry.cells);
      const kvK = kvPayRowKey(
        debtSpreadsheetId,
        BAO_CAO_TK_TAB_NAME,
        entry.sheetRow1Based,
        rowHash,
      );
      const sent = await store.get(kvK);
  ...
  for (let batchStart = 0; batchStart < paymentGroups.length; batchStart += SHEET_PAY_GROUP_BATCH_SIZE) {
    ...
    for (const group of batch) {
      ...
      for (const entry of entries) {
        ...
        const congNoSauMcc =
          bStr != null && tongThuNum > 0
            ? formatDebtDisplayForTelegram(
                formatMoneyForThanhToanLine(Math.round((bNum + tongThuNum) * 100) / 100),
              )
            : congNoColumnBForDealerName(debtMap, customerD);

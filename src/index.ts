...
async function handleApi(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  ...
  try {
    ...
    if (path === "/api/coc" && request.method === "PUT") {
      ...
      const { patchCocDataRows, COC_TAB_NAME } = await import("./coc-sheet");
      const rows = Array.isArray(body.rows) ? body.rows : [];
      if (rows.length === 0) {
        return json({ ok: false, error: "Không có dòng nào cần lưu." }, 400, request);
      }
      try {
        const token = await getAccessTokenFromEnv(env);
        const { thuChiSpreadsheetId } = await import("./thu-chi-hub");
        const written = await patchCocDataRows(token, thuChiSpreadsheetId(env), COC_TAB_NAME, rows);
        return json(
          { ok: true, message: `Đã cập nhật ${written} dòng tab COC (các dòng khác giữ nguyên).` },
          200,
          request,
        );
      } catch (e) {
    ...
    if (path === "/api/cham-cong" && request.method === "GET") {
      ...
      try {
        const { getChamCongEmployeeMap } = await import("./cham-cong-map");
        const {
          readChamCongRows,
          readChamCongTienUngMacDinh,
          ensureTodayDateRow,
        ...
        let tienUngMacDinh = "";
        if (tabName) {
          await ensureTodayDateRow(token, spreadsheetId, tabName);
          rows = await readChamCongRows(token, spreadsheetId, tabName);
          tienUngMacDinh = await readChamCongTienUngMacDinh(token, spreadsheetId, tabName);
        }
        return json(
          {
            ok: true,
    ...
    /** Chấm công — lưu tiền ứng mặc định (ô C2). */
    if (path === "/api/cham-cong" && request.method === "PUT") {
      if (!(await verifySessionCookie(request, env.SESSION_SECRET))) {
        return json({ ok: false, error: "Chưa đăng nhập" }, 401, request);
      }
      const body = (await request.json()) as { tabName?: string; tienUngMacDinh?: string | number };
      const tabName = String(body.tabName ?? "").trim();
      if (!tabName) {
        return json({ ok: false, error: "Thiếu tên tab." }, 400, request);
      }
      try {
        const { writeChamCongTienUngMacDinh } = await import("./cham-cong-sheet");
        const { chamCongSpreadsheetId } = await import("./cham-cong-hub");
        const token = await getAccessTokenFromEnv(env);
        await writeChamCongTienUngMacDinh(
          token,
          chamCongSpreadsheetId(env),
          tabName,
          body.tienUngMacDinh ?? "",
        );
        return json({ ok: true, message: `Đã lưu tiền ứng mặc định (C2) tab ${tabName}.` }, 200, request);
      } catch (e) {
        const msg = e instanceof Error ? e.message : String(e);
        return json({ ok: false, error: msg }, 502, request);
      }
 ...
 * Mỗi dòng → nhóm map cột D. Gửi theo lô 3 nhóm, nghỉ giữa các lô.
...
async function broadcastDealerTabs(
  ...
  const sendGroupFooter = async (chatId: string, customerD: string) => {
    ...
    try {
      ...
      /* ghi CONG_NO lỗi không hoàn tác Telegram */
    }
  };
  ...
  for (let batchStart = 0; batchStart < paymentGroups.length; batchStart += SHEET_PAY_GROUP_BATCH_SIZE) {
    ...
    for (const group of batch) {
      ...
      if (sentAnyRow) {
        await sendGroupFooter(chatId, customerD);
      }
    }

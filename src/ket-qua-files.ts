/**
 * Tính tiền đại lý từ nhiều file XLSX/CSV — so sánh tài khoản & chiến dịch giữa các file.
 */
import {
  colLettersToIdx,
  parseMoney,
...
export function analyzeKetQuaFiles(
  ...
  if (!files.length) {
    throw new Error("Chọn ít nhất một file XLSX hoặc CSV.");
  }
  ...
  return {
    ...
    campaignReportTxt,
    ketQuaCsv,
    summary: `Đã phân tích ${files.length} file. Tải 2 file TXT (tài khoản, chiến dịch) và CSV tổng hợp KET_QUA.`,
  };
}

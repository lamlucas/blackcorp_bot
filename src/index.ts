...
async function handleApi(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  ...
  try {
    ...
    /** Email — đọc danh sách từ KV (mỗi dòng một email). */
    if (path === "/api/mail-list" && request.method === "GET") {
      if (!(await verifySessionCookie(request, env.SESSION_SECRET))) {
        return json({ ok: false, error: "Chưa đăng nhập" }, 401, request);
      }
      const { getMailList } = await import("./mail-list-kv");
      const emails = await getMailList(env.STORE);
      return json({ ok: true, emails, text: emails.join("\n") }, 200, request);
    }

    /** Email — lưu danh sách lên KV. */
    if (path === "/api/mail-list" && request.method === "PUT") {
      if (!(await verifySessionCookie(request, env.SESSION_SECRET))) {
        return json({ ok: false, error: "Chưa đăng nhập" }, 401, request);
      }
      const body = (await request.json()) as { text?: string; emails?: string[] };
      const rawText =
        body.text != null
          ? String(body.text)
          : Array.isArray(body.emails)
            ...
            : "";
      try {
        const { setMailList } = await import("./mail-list-kv");
        const emails = await setMailList(env.STORE, rawText.split(/\r?\n/));
        return json(
          { ok: true, message: `Đã lưu ${emails.length} email.`, emails, text: emails.join("\n") },
          200,
          request,
        );
      } catch (e) {

...
/** @type {Set<number>} */
let cocDeletedSheetRows = new Set();
/** @type {Set<number>} */
let cocEditingSheetRows = new Set();
...
function rebuildCocOriginal(rows) {
  ...
  cocMaxSheetRow = 1;
  cocDeletedSheetRows = new Set();
  cocEditingSheetRows = new Set();
  for (const r of rows) {
...
function cocRowIsEditing(r) {
  if (r.isNew) return true;
  if (r.sheetRow && cocEditingSheetRows.has(r.sheetRow)) return true;
  return Boolean(r.editing);
}

function syncCocRowsFromDom() {
  const collected = collectCocRowsFromDom();
  for (let i = 0; i < cocRowsCache.length && i < collected.length; i++) {
    Object.assign(cocRowsCache[i], collected[i]);
  }
}
...
function renderCocTable(rows) {
  cocRowsCache = rows.map((r) => ({
    ...
    editing: Boolean(r.isNew) || (r.sheetRow ? cocEditingSheetRows.has(r.sheetRow) : false),
  }));
  renderCocTableFromCache();
}

function renderCocTableFromCache() {
  const tb = document.getElementById("cocTableBody");
  if (!tb) return;
  tb.innerHTML = "";
  for (let i = 0; i < cocRowsCache.length; i++) {
    const r = cocRowsCache[i];
    const editing = cocRowIsEditing(r);
    const tr = document.createElement("tr");
    tr.className = editing ? "coc-row-editing" : "coc-row-locked";
    if (r.sheetRow) tr.dataset.sheetRow = String(r.sheetRow);
    if (r.isNew) tr.dataset.cocNew = "1";
    ...
    for (const f of fields) {
      ...
      const val = f === "thu" || f === "chi" ? fmtMoneyDisplay(r[f]) : String(r[f] ?? "");
      input.value = val;
      if (!editing) input.readOnly = true;
      td.appendChild(input);
      tr.appendChild(td);
    }

    const tdAct = document.createElement("td");
    tdAct.className = "coc-row-actions";
    if (r.isNew) {
      const del = document.createElement("button");
      del.type = "button";
      del.className = "btn ghost btn-row-del";
      del.dataset.cocDel = String(i);
      del.textContent = "×";
      del.title = "Bỏ dòng mới";
      tdAct.appendChild(del);
    } else if (editing) {
      const cancel = document.createElement("button");
      cancel.type = "button";
      cancel.className = "btn ghost btn-sm";
      cancel.dataset.cocCancel = String(i);
...
function getCocRowsToSave() {
  ...
  for (const row of cocRowsCache) {
    ...
    if (!row.sheetRow || !cocEditingSheetRows.has(row.sheetRow)) continue;
    const orig = cocOriginalBySheetRow.get(row.sheetRow);
    if (!orig || cocRowDiffers(row, orig)) {
      patches.push({ sheetRow: row.sheetRow, ...payload });
    }
  }
  return {
    patches,
    appends,
    deletedRows: [...cocDeletedSheetRows],
  };
}
...
async function saveCocTable() {
  ...
  const { patches, appends, deletedRows } = getCocRowsToSave();
  if (patches.length === 0 && appends.length === 0 && deletedRows.length === 0) {
    if (msg) show(msg, "Không có dòng nào đang sửa hoặc thêm mới.", false);
    return;
  }
  const res = await fetch(url, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    credentials: "include",
    body: JSON.stringify({ rows: patches, appends, deletedRows }),
  });
  const data = await res.json().catch(() => ({}));
  if (!res.ok || !data.ok) {
    if (msg) show(msg, data.error || "Lưu lỗi.", true);
...
async function loadText() {
  try {
    ...
    if (t.sheetPayConfirmTitle) {
      const el = document.getElementById("sheetPayConfirmTitle");
      if (el) el.textContent = t.sheetPayConfirmTitle;
    }
    if (t.sheetPayConfirmYes) {
      const el = document.getElementById("sheetPayConfirmYes");
      if (el) el.textContent = t.sheetPayConfirmYes;
    }
    if (t.sheetPayConfirmNo) {
      const el = document.getElementById("sheetPayConfirmNo");
      if (el) el.textContent = t.sheetPayConfirmNo;
    }
...
document.getElementById("cocAddRow")?.addEventListener("click", () => {
  ...
  renderCocTableFromCache();
  const tb = document.getElementById("cocTableBody");
  const last = tb?.querySelector("tr:last-child input[data-coc=\"ngay\"]");
  last?.focus();
});
document.getElementById("cocTableBody")?.addEventListener("click", (e) => {
  const editBtn = e.target.closest("[data-coc-edit]");
  if (editBtn) {
    const idx = Number(editBtn.getAttribute("data-coc-edit"));
    const row = cocRowsCache[idx];
    if (row?.sheetRow) {
      cocEditingSheetRows.add(row.sheetRow);
      row.editing = true;
      renderCocTableFromCache();
      const tr = document.getElementById("cocTableBody")?.querySelectorAll("tr")[idx];
      tr?.querySelector("input[data-coc=\"ngay\"]")?.focus();
    }
    return;
  }
  const cancelBtn = e.target.closest("[data-coc-cancel]");
  if (cancelBtn) {
    const idx = Number(cancelBtn.getAttribute("data-coc-cancel"));
    const row = cocRowsCache[idx];
    if (row?.sheetRow) {
      cocEditingSheetRows.delete(row.sheetRow);
      const orig = cocOriginalBySheetRow.get(row.sheetRow);
      if (orig) {
        row.ngay = orig.ngay;
        row.thu = orig.thu;
  ...
  const delBtn = e.target.closest("[data-coc-del]");
  if (!delBtn) return;
  const idx = Number(delBtn.getAttribute("data-coc-del"));
  syncCocRowsFromDom();
  cocRowsCache.splice(idx, 1);
  renderCocTableFromCache();
});
...
document.getElementById("sendSheetPayment").addEventListener("click", () => {
...
document.getElementById("sheetPayConfirmYes")?.addEventListener("click", () => {
...
async function submitSheetPayment(forceResend, payloadOverride) {
  ...
  const dates =
    payloadOverride?.dates ??
    parseMultilineField(document.getElementById("sheetPayDates")?.value ?? "");
  const mccs =
    payloadOverride?.mccs ??
    parseMultilineField(document.getElementById("sheetPayMccs")?.value ?? "");
  const excludeMccs =
  ...
  const extra =
    Array.isArray(data.filterDates) && data.filterDates.length > 1
      ...
      : "";
  const resentHint = forceResend ? " (gửi lại)" : "";
  show(msg, (data.message || "Đã nhận — bot đang gửi thanh toán…") + resentHint + extra, false);
  if (data.runId) {
    void pollSheetPayStatus(data.runId, msg);
  }
}

async function pollSheetPayStatus(runId, msgEl) {
  const statusBase = apiUrl("/api/sheet-pay-status");
  if (!statusBase || !runId || !msgEl) return;
  ...
  for (let i = 0; i < maxAttempts; i++) {
    ...
    const res = await fetch(
      `${statusBase}?runId=${encodeURIComponent(runId)}`,
      { credentials: "include" },
    );
    const data = await res.json().catch(() => ({}));
    ...
    const run = data.run;
    if (!run || run.runId !== runId) continue;

    const errors = Array.isArray(run.errors) ? run.errors : [];
    const warnings = Array.isArray(run.warnings) ? run.warnings : [];
    const footers = Array.isArray(run.footersSent) ? run.footersSent : [];

    if (!run.complete) {
      const total = run.totalRowJobs ?? 0;
      const sent = run.rowsSent ?? 0;
      const progress = total > 0 ? ` (${sent}/${total} dòng chi phí)` : "";
      const hint =
        footers.length > 0
          ? ` — đã gửi TỔNG TIỀN + QR: ${footers.join(", ")}`
          : "";
      show(msgEl, `Bot đang gửi chi phí / TỔNG TIỀN…${progress}${hint}`, false);
      continue;
    }
    ...
    show(
      msgEl,
      footers.length
        ? `Hoàn tất — đã gửi TỔNG TIỀN + QR: ${footers.join(", ")}`
        : "Hoàn tất — không có TỔNG TIỀN + QR (tổng ≤ 0 hoặc không khớp nợ).",
      false,
    );
    return;
  }
  ...
  show(
    msgEl,
    "Chưa nhận được trạng thái hoàn tất — có thể Worker timeout. Kiểm tra Telegram hoặc thử giảm số dòng gửi.",
    true,
  );
}
...
function fillMailListInputs(data) {
  const admin = document.getElementById("mailListAdmin");
  const chuan = document.getElementById("mailListChuan");
  const readonly = document.getElementById("mailListReadonly");
  if (admin) admin.value = data.admin ?? data.groups?.admin?.join("\n") ?? "";
  if (chuan) chuan.value = data.chuan ?? data.groups?.chuan?.join("\n") ?? "";
  if (readonly) readonly.value = data.readonly ?? data.groups?.readonly?.join("\n") ?? "";
}

function clearMailListInputs() {
  fillMailListInputs({ admin: "", chuan: "", readonly: "" });
}
...
async function saveMailListPanel() {
  ...
  const url = apiUrl("/api/mail-list");
  const payload = {
    admin: document.getElementById("mailListAdmin")?.value ?? "",
    chuan: document.getElementById("mailListChuan")?.value ?? "",
    readonly: document.getElementById("mailListReadonly")?.value ?? "",
  };
  if (!url) {
    if (msg) show(msg, "Chỉ lưu được khi chạy trên server.", true);
    return;

...
import { parseSpreadsheetFile } from "./ket-qua-file-parse.js?v=1";
...
const ADMIN_TABS = ["dealers", "chat-filter", "sheet-pay", "coc", "debt-notify", "ket-qua", "ket-qua-files", "broadcast"];
...
function collectCocRowsFromDom() {
  ...
  return rows.filter((r) => r.ngay || r.ten || r.note || r.thu || r.chi);
}
...
async function loadText() {
  try {
    ...
    if (t.ketQuaFilesLblUpload)
      document.getElementById("ketQuaFilesLblUpload").textContent = t.ketQuaFilesLblUpload;
    ...
    if (t.ketQuaFilesDownloadsHint)
      document.getElementById("ketQuaFilesDownloadsHint").textContent = t.ketQuaFilesDownloadsHint;
...
document.getElementById("cocAddRow")?.addEventListener("click", () => {
  const rows = collectCocRowsFromDom();
  rows.push({ ngay: "", thu: "", chi: "", ten: "", note: "" });
  renderCocTable(rows);
});
...
let ketQuaFilesLastResult = null;
...
function updateKetQuaFilesList() {
  const input = document.getElementById("ketQuaFilesInput");
  const listEl = document.getElementById("ketQuaFilesList");
  ...
  listEl.hidden = false;
  listEl.textContent = `Đã chọn ${files.length} file: ${files.map((f) => f.name).join(", ")}`;
}

document.getElementById("ketQuaFilesInput").addEventListener("change", updateKetQuaFilesList);

document.getElementById("ketQuaFilesDlAccount").addEventListener("click", () => {
  if (!ketQuaFilesLastResult?.accountReportTxt) return;
  downloadBlob("bao-cao-tai-khoan.txt", ketQuaFilesLastResult.accountReportTxt);
});

document.getElementById("ketQuaFilesDlCampaign").addEventListener("click", () => {
  if (!ketQuaFilesLastResult?.campaignReportTxt) return;
  downloadBlob("bao-cao-chien-dich.txt", ketQuaFilesLastResult.campaignReportTxt);
});

document.getElementById("ketQuaFilesDlCsv").addEventListener("click", () => {
  if (!ketQuaFilesLastResult?.ketQuaCsv) return;
  downloadBlob("KET_QUA.csv", ketQuaFilesLastResult.ketQuaCsv, "text/csv;charset=utf-8");
});

document.getElementById("ketQuaFilesForm").addEventListener("submit", async (e) => {
  e.preventDefault();
  const msg = document.getElementById("ketQuaFilesMsg");
  const dl = document.getElementById("ketQuaFilesDownloads");
  const runBtn = document.getElementById("ketQuaFilesRun");
  msg.hidden = true;
  dl.hidden = true;
  ketQuaFilesLastResult = null;

  const url = apiUrl("/api/analyze-ket-qua-files");
  if (!url) {
    show(msg, "Chỉ chạy được khi site trên Worker.", true);
    return;
  }
  ...
  if (!fileList.length) {
    show(msg, "Chọn ít nhất một file XLSX hoặc CSV.", true);
    return;
  }

  runBtn.disabled = true;
  show(msg, "Đang đọc file…", false);
  msg.hidden = false;
  ...
  try {
    ...
    for (const f of fileList) {
      ...
      parsed.push({ fileName: f.name, rows });
    }
  } catch (err) {
    runBtn.disabled = false;
    show(msg, err instanceof Error ? err.message : String(err), true);
    return;
  }

  show(msg, "Đang phân tích trên máy chủ…", false);
  ...
  if (!res.ok || !data.ok) {
    show(msg, data.error || "Phân tích thất bại", true);
    return;
  }

  ketQuaFilesLastResult = data;
  dl.hidden = false;
  show(msg, data.summary || "Đã xong.", false);
});

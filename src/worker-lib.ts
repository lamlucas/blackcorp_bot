...
export interface Env {
   ...
   * Tab DEBT_TAB_NAME (vd CONG_NO): cột A = mã / tên đại lý (khớp « Tên đại lý » form KV); cột B = nợ.
  ...
  /** "0" / "false" — tắt cron getUpdates (Thu: / CONG_NO). Mặc định bật. */
  ...
  /** Khớp header X-Telegram-Bot-Api-Secret-Token khi dùng POST /api/telegram-webhook */
...
/** Ghi ô (cột + dòng) — ví dụ cột J = TỔNG THU theo dòng dữ liệu. Cần quyền Editor. */

...
async function broadcastDealerTabs(
  ...
  const writeRowNote = async (sheetRow: number, status: "Done" | "Error") => {
    try {
      await writeSheetCell(token, debtSpreadsheetId, BAO_CAO_TK_TAB_NAME, "I", sheetRow, status);
    } catch {
  ...
  for (const entry of dataRows) {
    ...
    const incomplete = isBaoCaoRowPaymentIncomplete(row);

    if (!chatId || incomplete) {
      await writeRowNote(sheetRow, "Error");
      continue;
    }
  ...
  for (let batchStart = 0; batchStart < paymentGroups.length; batchStart += SHEET_PAY_GROUP_BATCH_SIZE) {
    ...
    for (const group of batch) {
      ...
      for (const entry of entries) {
        ...
        const tongThuDisplay =
          String(row[BAO_CAO_COL.TONG_THU] ?? row[8] ?? "").trim() || "0";
